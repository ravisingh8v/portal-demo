...
# -----------------------
# STEP 3 — CREATE TEMP WORKTREE FOR REPLIT BRANCH
# -----------------------
...
# -----------------------
# STEP 5 — CLEAN OLD FRONTEND (optional)
# -----------------------
if [ -d "./src" ]; then
  rm -rf ./src
...
# Copy all root-level files except excluded ones
for file in "$TEMP_DIR"/*; do
  name=$(basename "$file")
  is_excluded=false

  for excluded_item in "${EXCLUDES_ROOT[@]}"; do
    if [[ "$name" == "$excluded_item" ]]; then
      is_excluded=true
      break
    fi
  done
...
# -----------------------
# STEP 7 — REMOVE BACKEND FILES
# -----------------------
echo "🗑️ Removing backend files/folders from root..."
